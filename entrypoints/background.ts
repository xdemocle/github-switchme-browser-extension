  ...
  browser.runtime.onMessage.addListener((message: Message, sender, sendResponse) => {
    ...
    switch (message.type) {
      case MessageType.GET_ACCOUNTS:
        accountManager.getAccounts().then(accounts => {
      ...
      case MessageType.ADD_ACCOUNT:
        if (message.payload && message.payload.account) {
          accountManager.storeAccount(message.payload.account).then(() => {
      ...
      case MessageType.SWITCH_ACCOUNT:
        if (message.payload && message.payload.username) {
          accountManager.setCurrentAccount(message.payload.username).then(() => {
            sendResponse({ success: true });
          }).catch(error => {
